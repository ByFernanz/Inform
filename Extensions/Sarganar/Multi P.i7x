Version 1/090121 of Multi P (for Glulx only) by Sarganar begins here.
...
*ojo al usar tell combinado con say en una rule: Tell, cierra el tags y lo s siguientes says salen como PUBLICO. BUG?
...
When play begins: move yourself to the Recibidor.
...
A PC can be action-observant. The action-observant property translates into I6 as "i7_action_observant".
...
To announce (message - indexed text): [un broadcast generico]
...
To add a player named (new mud-name - indexed text) with ID (new mud-id - indexed text) at location (new path - indexed text):
	...
	otherwise if the uncase-name names a connected PC;
		won new mud-id;
		say "El nombre '[new mud-name]' ya está en uso.";
		woff;
...
[		now the newbie is unreserved;]
		...
		change the mud-id of the newbie to the new mud-id;
		change the mud-name of the newbie to the new mud-name;
		change the low-mud-name of the newbie to the uncase-name;
		follow the player joining rules for the newbie;
	end if.
...
The last player leaving rule (this is the low-level PC cleanup rule):
	...
	change the mud-id of the current PC to "";
	change the mud-name of the current PC to "";
	[change the low-mud-name of the current PC to "";]
...
To cleanup (X - a PC):
	change the player to yourself;
	change the mud-id of X to "";
	change the mud-name of X to "";
	move X to the PC-corral.

To transfer (character - a PC) to (destiny - a room):
	change the player to the character;
...
Instead of helping (this is the standard helping rule):
	say "Comandos básicos [line break]
      - para hablar: los tipicos de inform [line break]
      ...
      - para salir: quit (salir del juego, no matar el programa...)[line break]
...
Instead of asking the noun about something (this is the spanish standard chatting rule):
	tell "Dices, '[topic understood]'" to the player;
...
Report an actor waving when the actor is not the player (this is the spanish standard report someone waving hands rule):
